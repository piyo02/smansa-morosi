...
class Ion_auth_model extends MY_Model
{
	 ...
	 * @author Ben Edmunds
	 */
	public function users($group_ids = NULL)
	{
		...
		if (isset($this->_ion_select) && !empty($this->_ion_select)) {
			...
			$this->db->select([
				...
				'CONCAT( table_users.first_name, " ", table_users.last_name ) as user_fullname',
				'table_groups.description  as group_name',
			]);
			$this->db->join(
				"table_groups",
				"table_groups.id = table_users.group_id",
				"inner"
			);
		}
		...
		if (isset($group_ids)) {
			...
			$this->db->where_in($this->tables['users'] . '.group_id', $group_ids);
		}
	 ...
	 * @author madukubah
	 */
	public function member_users_limit($limit,  $start = 0, $groups = NULL)
	{
	...
	{
		return  $this->db->where($this->tables['users'] . '.group_id', 2)
	 ...
	 * @author madukubah
	 */
	public function search_member($query)
	{

		// $this->like( $this->tables['users'].'.username', $query );
		// $this->like( $this->tables['users'].'.first_name', $query );
		// $this->like( $this->tables['users'].'.last_name', $query );
		// $this->like( $this->tables['users'].'.email', $query );
		// $this->users( 2 );
		// return $this;
		$sql = "
			SELECT `table_users`.*, `table_users`.`id` as `id`, `table_users`.`id` as `user_id`, CONCAT( table_users.first_name, ' ', table_users.last_name ) as user_fullname, `table_groups`.`description` as `group_name` FROM `table_users` 
			INNER JOIN `table_groups` ON `table_groups`.`id` = `table_users`.`group_id` 
			WHERE `table_users`.`group_id` IN(2) 
			AND
			(
				`table_users`.`username` LIKE '%" . $query . "%' ESCAPE '!' 
				OR `table_users`.`first_name` LIKE '%" . $query . "%' ESCAPE '!'  
				OR `table_users`.`last_name` LIKE '%" . $query . "%' ESCAPE '!' 
				OR `table_users`.`email` LIKE '%" . $query . "%' ESCAPE '!'
			)
		";
	...
	{
		return $this->session->userdata('group_id') == $group_id;
	}
